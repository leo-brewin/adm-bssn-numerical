...
package BSSNBase is
   ...
   type TraceKGridArray   is Array (Integer range <>, Integer range <>, Integer range <>) of Real;
   type ConFactGridArray  is Array (Integer range <>, Integer range <>, Integer range <>) of Real;
   ...
   type GammaGridArray_ptr   is access GammaGridArray;
   ...
   type TraceKGridArray_ptr  is access TraceKGridArray;
   type ConFactGridArray_ptr is access ConFactGridArray;
   ...
   gBar_ptr     : MetricGridArray_ptr  := new MetricGridArray  (1..max_num_x, 1..max_num_y, 1..max_num_z);
   ABar_ptr     : ExtcurvGridArray_ptr := new ExtcurvGridArray (1..max_num_x, 1..max_num_y, 1..max_num_z);
   N_ptr        : LapseGridArray_ptr   := new LapseGridArray   (1..max_num_x, 1..max_num_y, 1..max_num_z);
   trK_ptr      : TraceKGridArray_ptr  := new TraceKGridArray  (1..max_num_x, 1..max_num_y, 1..max_num_z);
   phi_ptr      : ConFactGridArray_ptr := new ConFactGridArray (1..max_num_x, 1..max_num_y, 1..max_num_z);
   Gi_ptr       : GammaGridArray_ptr   := new GammaGridArray   (1..max_num_x, 1..max_num_y, 1..max_num_z);

   gBar         : MetricGridArray  renames gBar_ptr.all;
   ABar         : ExtcurvGridArray renames ABar_ptr.all;
   N            : LapseGridArray   renames N_ptr.all;
   trK          : TraceKGridArray  renames trK_ptr.all;
   phi          : ConFactGridArray renames phi_ptr.all;
   Gi           : GammaGridArray   renames Gi_ptr.all;

   dot_gBar_ptr : MetricGridArray_ptr  := new MetricGridArray  (1..max_num_x, 1..max_num_y, 1..max_num_z);
   dot_ABar_ptr : ExtcurvGridArray_ptr := new ExtcurvGridArray (1..max_num_x, 1..max_num_y, 1..max_num_z);
   dot_N_ptr    : LapseGridArray_ptr   := new LapseGridArray   (1..max_num_x, 1..max_num_y, 1..max_num_z);
   dot_trK_ptr  : TraceKGridArray_ptr  := new TraceKGridArray  (1..max_num_x, 1..max_num_y, 1..max_num_z);
   dot_phi_ptr  : ConFactGridArray_ptr := new ConFactGridArray (1..max_num_x, 1..max_num_y, 1..max_num_z);
   dot_Gi_ptr   : GammaGridArray_ptr   := new GammaGridArray   (1..max_num_x, 1..max_num_y, 1..max_num_z);

   dot_gBar     : MetricGridArray  renames dot_gBar_ptr.all;
   dot_ABar     : ExtcurvGridArray renames dot_ABar_ptr.all;
   dot_N        : LapseGridArray   renames dot_N_ptr.all;
   dot_trK      : TraceKGridArray  renames dot_trK_ptr.all;
   dot_phi      : ConFactGridArray renames dot_phi_ptr.all;
   dot_Gi       : GammaGridArray   renames dot_Gi_ptr.all;
   ...
   the_time : Real := 0.0;
   beg_time : Real := 0.0;  -- used only by bssninitial
   end_time : Real := 1.0;  -- used only by bssnevolve
